...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['add_quiz'])) {
    ...
    if (!in_array($post_course_id, $myCourseIds)) {
        ...
        $stmt = $conn->prepare("INSERT INTO quizzes (course_id, title, description, due_date, created_by, created_by_role) VALUES (?, ?, ?, ?, ?, 'lecturer')");
        $stmt->bind_param("isssi", $post_course_id, $title, $desc, $due, $lecturer_id);
        $stmt->execute();
